...
public extension String.Utf8 {
    ...
    struct Mutable {
        ...
        public var raw: fge_utf8_pt
        
        @usableFromInline
        init(_ raw: fge_utf8_pt) {
            self.raw = raw
        }
...
extension String.Utf8.Mutable : IUnsafeString {
...
extension String.Utf8.Mutable : IString {
...
extension String.Utf8.Mutable : IMutableString {
    ...
    public func remove(in range: String.Range) -> Bool {
        return fge_utf8_remove_range(self.raw, range.raw)
    }
    ...
    public func replace< First : IUnsafeString, To : IUnsafeString >(first: First, to: To, in range: String.Range) -> Bool {
        first.unsafe({ firstPtr, firstSize in
            to.unsafe({ toPtr, toSize in
                fge_utf8_replace_first(
                    self.raw,
                    range.raw,
                    .init(
                        ptr: fge_utf8_unit_cpt(firstPtr),
                        count: fge_size_t(firstSize)
                    ),
                    .init(
                        ptr: fge_utf8_unit_cpt(toPtr),
                        count: fge_size_t(toSize)
                    )
                )
            })
        })
    ...
    public func replace< Last : IUnsafeString, To : IUnsafeString >(last: Last, to: To, in range: String.Range) -> Bool {
        last.unsafe({ lastPtr, lastSize in
            to.unsafe({ toPtr, toSize in
                fge_utf8_replace_last(
                    self.raw,
                    range.raw,
                    .init(
                        ptr: fge_utf8_unit_cpt(lastPtr),
                        count: fge_size_t(lastSize)
                    ),
                    .init(
                        ptr: fge_utf8_unit_cpt(toPtr),
                        count: fge_size_t(toSize)
    ...
    public func replace< All : IUnsafeString, To : IUnsafeString >(all: All, to: To, in range: String.Range) -> UInt {
        all.unsafe({ allPtr, allSize in
            to.unsafe({ toPtr, toSize in
                fge_utf8_replace_all(
                    self.raw,
                    range.raw,
                    .init(
                        ptr: fge_utf8_unit_cpt(allPtr),
                        count: fge_size_t(allSize)
                    ),
                    .init(
                        ptr: fge_utf8_unit_cpt(toPtr),
                        count: fge_size_t(toSize)
