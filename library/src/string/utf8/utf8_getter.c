...
fge_size_t fge_utf8_count(
    fge_utf8_cpt self
) {
    return self->count;
}

fge_size_t fge_utf8_capacity(
    fge_utf8_cpt self
) {
    return self->capacity;
}

fge_index_range_t fge_utf8_range(
    fge_utf8_cpt self
) {
    return (fge_index_range_t){ .lower = 0, .upper = self->count };
}

fge_index_t fge_utf8_start_index(
    fge_utf8_cpt self
) {
    return 0;
}

fge_index_t fge_utf8_end_index(
    fge_utf8_cpt self
) {
    return self->count;
}
