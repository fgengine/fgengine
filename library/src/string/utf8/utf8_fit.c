...
fge_bool_t fge_utf8_fit(
    ...
    if (self->count >= sizeof(fge_ptr_t)) {
        ...
        fge_utf8_unit_pt new_ptr = self->allocator->new(new_count * sizeof(fge_utf8_unit_t));
        if (new_ptr == NULL) {
            return false;
        }
        fge_memory_copy((fge_memory_t){
            .ptr = new_ptr,
            .count = new_count * sizeof(fge_utf8_unit_t)
        }, (fge_memory_t){
            .ptr = self->ptr,
            .count = self->capacity * sizeof(fge_utf8_unit_t)
        });
        self->ptr = new_ptr;
