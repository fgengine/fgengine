#include "utf16_string.h"

fge_bool_t fge_utf16_is_suffix(
    fge_utf16_cpt self,
    fge_utf16_buffer_t other
) {
    if (self->count < other.count) {
        return false;
    }
    return fge_memory_is_equal((fge_memory_t){
        .ptr = fge_utf16_pointer(self) + (self->count - other.count),
        .count = other.count * sizeof(fge_utf16_unit_t)
    }, (fge_memory_t){
        .ptr = (fge_ptr_t)other.ptr,
        .count = other.count * sizeof(fge_utf16_unit_t)
    });
}

fge_bool_t fge_utf16_is_suffix_string(
    fge_utf16_cpt self,
    fge_utf16_cpt other
) {
    return fge_utf16_is_suffix(self, (fge_utf16_buffer_t){
        .ptr = fge_utf16_pointer(other),
        .count = fge_utf16_count(other)
    });
}
